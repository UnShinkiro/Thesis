...
for speaker in spk_list:
    ...
    for file in utterance[speaker]['files']:
        ...
        support_threshold = np.mean(scores)
        ...
        for n in range(N_MODELS):
            ...
            for d_utterance in d_vectors[n]['d_utterance_list']:
                ...
                score.append(corr)
                if support_threshold > THRESHOLD:
                    if corr > THRESHOLD:
                    ...
                    if corr > THRESHOLD:
                        ...
                        weight.append(abs(support_threshold/2 - abs(corr)))
            #weight = np.absolute(support_threshold-np.absolute(score))
...
with open('uncertainty.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
